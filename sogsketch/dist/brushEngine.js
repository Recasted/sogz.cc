const base = { size: 24, opacity: 100, flow: 100, hardness: 90, spacing: 10, smoothing: 35, stabilizer: 4, pressureCurve: 50, rotation: 0, angle: 0, randomSize: 0, randomOpacity: 0, randomRotation: 0, blendStrength: 55, tip: "round", texture: "none" };
...
export const brushCategories = ["Core Brushes", "Specialty Brushes", "Custom"];
...
export function presetToSettings(preset, current) { return { ...current, ...preset, pressureCurve: preset.pressureCurve ?? current.pressureCurve ?? 50, eraserMode: false, symmetry: current.symmetry }; }
...
const effectTips = new Set(["scatter", "leaf", "grass", "cloud"]);
const pressureValue = (raw, curve) => {
...
export class StrokeRenderer {
    ...
    get usesStampPath() { return effectTips.has(this.options.settings.tip) || this.options.settings.hardness < 45; }
    ...
    drawStart(point) { const s = this.options.settings, p = pressureValue(point.pressure, s.pressureCurve), radius = Math.max(.25, s.size * p / 2), alpha = (s.opacity / 100) * (s.flow / 100); this.ctx.save(); this.options.clip?.(this.ctx); this.ctx.globalCompositeOperation = this.options.erase ? "destination-out" : "source-over"; this.ctx.globalAlpha = alpha; this.ctx.fillStyle = this.options.color; this.ctx.beginPath(); if (s.tip === "flat") {
        const angle = (s.angle + s.rotation) * Math.PI / 180, w = radius, h = Math.max(.5, radius * .24), c = Math.cos(angle), n = Math.sin(angle);
        this.ctx.ellipse(point.x, point.y, w, h, angle, 0, Math.PI * 2);
    }
    else
    ...
    drawContinuous(a, b) {
        ...
        if (s.tip === "flat") {
            ...
            wa = Math.max(.4, ra * projection);
            wb = Math.max(.4, rb * projection);
        }
        this.ctx.save();
        ...
        this.ctx.beginPath();
        if (s.tip === "flat")
            this.ctx.ellipse(b.x, b.y, rb, Math.max(.4, rb * .32), angle, 0, Math.PI * 2);
        else
    ...
    emitSegment(a, b, finish = false) {
        ...
        if (finish && this.carry > Math.max(.5, this.options.settings.size * .08)) {
            this.paint(b, direction);
            this.carry = 0;
        }
    }
    paint(point, direction, initial = false) {
...
export class SmudgeRenderer {
    ...
    previous = null;
    carry = 0;
    pickup = document.createElement("canvas");
    mask = document.createElement("canvas");
    constructor(ctx, settings, clip) {
        this.ctx = ctx;
        this.settings = settings;
        this.clip = clip;
    }
    begin(point) { this.previous = { ...point }; this.capture(point); }
    add(point) { if (!this.previous) {
        this.begin(point);
        return;
    } const a = this.previous, distance = Math.hypot(point.x - a.x, point.y - a.y); if (distance < .01)
        return; const step = Math.max(1, this.settings.size * Math.max(.05, this.settings.spacing / 100) * .45); let d = step - this.carry; while (d <= distance) {
        const t = d / distance;
        this.dab({ x: a.x + (point.x - a.x) * t, y: a.y + (point.y - a.y) * t, pressure: a.pressure + (point.pressure - a.pressure) * t });
        d += step;
    } this.carry = Math.max(0, distance - (d - step)); this.previous = { ...point }; }
    end(point) { if (point)
        this.add(point); this.previous = null; }
    ensure(size) { if (this.pickup.width === size)
        return; this.pickup.width = this.pickup.height = this.mask.width = this.mask.height = size; const m = this.mask.getContext("2d"), r = size / 2, g = m.createRadialGradient(r, r, 0, r, r, r); g.addColorStop(0, "rgba(255,255,255,1)"); g.addColorStop(Math.max(.05, this.settings.hardness / 100), "rgba(255,255,255,.94)"); g.addColorStop(1, "rgba(255,255,255,0)"); m.fillStyle = g; m.fillRect(0, 0, size, size); }
    capture(point) { const size = Math.max(4, Math.ceil(this.settings.size * pressureValue(point.pressure, this.settings.pressureCurve))); this.ensure(size); const p = this.pickup.getContext("2d"); p.clearRect(0, 0, size, size); p.drawImage(this.ctx.canvas, point.x - size / 2, point.y - size / 2, size, size, 0, 0, size, size); p.globalCompositeOperation = "destination-in"; p.drawImage(this.mask, 0, 0); p.globalCompositeOperation = "source-over"; }
    dab(point) { const size = this.pickup.width; if (!size)
        return; const strength = (this.settings.blendStrength / 100) * pressureValue(point.pressure, this.settings.pressureCurve); this.ctx.save(); this.clip?.(this.ctx); this.ctx.globalAlpha = Math.min(.92, strength); this.ctx.drawImage(this.pickup, point.x - size / 2, point.y - size / 2); this.ctx.restore(); const p = this.pickup.getContext("2d"); p.globalAlpha = Math.max(.08, 1 - strength * .72); p.globalCompositeOperation = "source-over"; p.drawImage(this.ctx.canvas, point.x - size / 2, point.y - size / 2, size, size, 0, 0, size, size); p.globalAlpha = 1; p.globalCompositeOperation = "destination-in"; p.drawImage(this.mask, 0, 0); p.globalCompositeOperation = "source-over"; }
}
