...
function renderReferenceList() { if (!references.length) {
    ...
    return;
} referenceList.replaceChildren(...references.map(reference => { const button = document.createElement("button"); button.className = `reference-entry${reference.id === selectedReferenceId ? " active" : ""}`; button.title = `Select ${reference.name}`; const thumb = document.createElement("img"); thumb.src = reference.src; thumb.alt = ""; const name = document.createElement("span"); name.textContent = reference.name; const visible = document.createElement("i"); visible.textContent = reference.visible ? "◉" : "○"; button.append(thumb, name, visible); button.addEventListener("click", () => { selectedReferenceId = reference.id; renderReferences(); }); return button; })); syncReferenceControls(); }
function renderReferences() { referenceOverlay.replaceChildren(...references.map(reference => { const item = document.createElement("div"); item.className = `reference-item${reference.id === selectedReferenceId ? " selected" : ""}`; item.dataset.id = String(reference.id); item.style.left = `${reference.x}px`; item.style.top = `${reference.y}px`; item.style.width = `${reference.baseWidth * reference.scale / 100}px`; item.style.height = `${reference.baseHeight * reference.scale / 100}px`; item.style.opacity = String(reference.opacity / 100); item.style.display = reference.visible ? "block" : "none"; const image = document.createElement("img"); image.src = reference.src; image.alt = reference.name; image.draggable = false; item.append(image); item.addEventListener("pointerdown", event => { if (!referenceEditing)
    return; event.preventDefault(); event.stopPropagation(); selectedReferenceId = reference.id; referenceDrag = { pointerId: event.pointerId, id: reference.id, startX: event.clientX, startY: event.clientY, originX: reference.x, originY: reference.y }; item.setPointerCapture(event.pointerId); referenceOverlay.querySelectorAll(".reference-item").forEach(node => node.classList.toggle("selected", node === item)); renderReferenceList(); }); item.addEventListener("pointermove", event => { if (!referenceDrag || referenceDrag.pointerId !== event.pointerId || referenceDrag.id !== reference.id)
