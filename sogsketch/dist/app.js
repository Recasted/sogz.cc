...
const referenceOverlay = qs("#referenceOverlay"), referenceList = qs("#referenceList"), referenceFileInput = qs("#referenceFileInput"), referenceOpacity = qs("#referenceOpacity"), referenceScale = qs("#referenceScale");
...
const brushSizeToSlider = (size, max = 300) => Math.round(Math.log(clamp(size, 1, max)) / Math.log(max) * 1000), sliderToBrushSize = (value, max = 300) => Math.max(1, Math.round(Math.exp(value / 1000 * Math.log(max))));
...
function selectedReference() { return references.find(item => item.id === selectedReferenceId) ?? null; }
function syncReferenceControls() { const reference = selectedReference(), controls = qs("#referenceControls"), edit = qs("#editReferences"); controls.hidden = !reference; edit.classList.toggle("active", referenceEditing); edit.setAttribute("aria-pressed", String(referenceEditing)); edit.textContent = referenceEditing ? "Done moving" : "Move / resize"; if (!reference)
    return; referenceOpacity.value = String(reference.opacity); referenceScale.value = String(reference.scale); qs("#referenceOpacityValue").value = `${reference.opacity}%`; qs("#referenceScaleValue").value = `${reference.scale}%`; qs("#toggleReferenceVisibility").textContent = reference.visible ? "Hide" : "Show"; }
function renderReferenceList() { if (!references.length) {
    const empty = document.createElement("p");
    empty.className = "reference-empty";
    empty.textContent = "Add images to use as tracing references.";
    referenceList.replaceChildren(empty);
    syncReferenceControls();
    return;
} referenceList.replaceChildren(...references.map(reference => { const row = document.createElement("div"); row.className = `reference-entry${reference.id === selectedReferenceId ? " active" : ""}`; const select = document.createElement("button"); select.className = "reference-select"; select.title = `Select ${reference.name}`; const thumb = document.createElement("img"); thumb.src = reference.src; thumb.alt = ""; const name = document.createElement("span"); name.textContent = reference.name; select.append(thumb, name); select.addEventListener("click", () => { selectedReferenceId = reference.id; renderReferences(); }); const visibility = document.createElement("button"); visibility.className = "reference-visibility"; visibility.title = reference.visible ? "Hide reference" : "Show reference"; visibility.setAttribute("aria-label", visibility.title); visibility.textContent = reference.visible ? "◉" : "○"; visibility.addEventListener("click", () => { selectedReferenceId = reference.id; reference.visible = !reference.visible; renderReferences(); }); row.append(select, visibility); return row; })); syncReferenceControls(); }
function renderReferences() { referenceOverlay.replaceChildren(...references.map(reference => { const item = document.createElement("div"); item.className = `reference-item${reference.id === selectedReferenceId ? " selected" : ""}`; item.dataset.id = String(reference.id); item.style.left = `${reference.x}px`; item.style.top = `${reference.y}px`; item.style.width = `${reference.baseWidth * reference.scale / 100}px`; item.style.height = `${reference.baseHeight * reference.scale / 100}px`; item.style.opacity = String(reference.opacity / 100); item.style.display = reference.visible ? "block" : "none"; const image = document.createElement("img"); image.src = reference.src; image.alt = reference.name; image.draggable = false; const handle = document.createElement("span"); handle.className = "reference-resize-handle"; handle.title = "Drag to resize"; item.append(image, handle); item.addEventListener("pointerdown", event => { if (!referenceEditing || event.target === handle)
    return; event.preventDefault(); event.stopPropagation(); selectedReferenceId = reference.id; referenceDrag = { pointerId: event.pointerId, id: reference.id, kind: "move", startX: event.clientX, startY: event.clientY, originX: reference.x, originY: reference.y, originScale: reference.scale }; item.setPointerCapture(event.pointerId); referenceOverlay.querySelectorAll(".reference-item").forEach(node => node.classList.toggle("selected", node === item)); renderReferenceList(); }); item.addEventListener("pointermove", event => { if (!referenceDrag || referenceDrag.kind !== "move" || referenceDrag.pointerId !== event.pointerId || referenceDrag.id !== reference.id)
    return; reference.x = referenceDrag.originX + (event.clientX - referenceDrag.startX) / state.view.zoom; reference.y = referenceDrag.originY + (event.clientY - referenceDrag.startY) / state.view.zoom; item.style.left = `${reference.x}px`; item.style.top = `${reference.y}px`; }); item.addEventListener("pointerup", event => { if (referenceDrag?.pointerId === event.pointerId) {
    referenceDrag = null;
    if (item.hasPointerCapture(event.pointerId))
        item.releasePointerCapture(event.pointerId);
} }); handle.addEventListener("pointerdown", event => { if (!referenceEditing)
    return; event.preventDefault(); event.stopPropagation(); selectedReferenceId = reference.id; referenceDrag = { pointerId: event.pointerId, id: reference.id, kind: "resize", startX: event.clientX, startY: event.clientY, originX: reference.x, originY: reference.y, originScale: reference.scale }; handle.setPointerCapture(event.pointerId); referenceOverlay.querySelectorAll(".reference-item").forEach(node => node.classList.toggle("selected", node === item)); renderReferenceList(); }); handle.addEventListener("pointermove", event => { if (!referenceDrag || referenceDrag.kind !== "resize" || referenceDrag.pointerId !== event.pointerId || referenceDrag.id !== reference.id)
    return; const dx = (event.clientX - referenceDrag.startX) / state.view.zoom, dy = (event.clientY - referenceDrag.startY) / state.view.zoom, change = Math.max(dx / reference.baseWidth, dy / reference.baseHeight) * 100; reference.scale = clamp(referenceDrag.originScale + change, 10, 500); item.style.width = `${reference.baseWidth * reference.scale / 100}px`; item.style.height = `${reference.baseHeight * reference.scale / 100}px`; referenceScale.value = String(Math.round(reference.scale)); qs("#referenceScaleValue").value = `${Math.round(reference.scale)}%`; }); handle.addEventListener("pointerup", event => { if (referenceDrag?.pointerId === event.pointerId) {
    referenceDrag = null;
    if (handle.hasPointerCapture(event.pointerId))
        handle.releasePointerCapture(event.pointerId);
} }); return item; })); renderReferenceList(); }
function setReferenceEditing(editing) { referenceEditing = editing && references.length > 0; referenceOverlay.classList.toggle("editing", referenceEditing); document.body.classList.toggle("reference-editing", referenceEditing); syncReferenceControls(); setStatus(referenceEditing ? "Drag reference images on the canvas" : "Reference locked for tracing"); }
async function fileAsDataUrl(file) { return await new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = () => reject(reader.error ?? new Error("Reference image could not be read")); reader.readAsDataURL(file); }); }
async function addReferenceFiles(files) { for (const file of files) {
    if (!file.type.startsWith("image/"))
        continue;
    const src = await fileAsDataUrl(file), image = await imageFromUrl(src), fit = Math.min(1, state.width * .7 / image.naturalWidth, state.height * .7 / image.naturalHeight), baseWidth = Math.max(20, image.naturalWidth * fit), baseHeight = Math.max(20, image.naturalHeight * fit), reference = { id: nextReferenceId++, name: file.name, src, x: (state.width - baseWidth) / 2, y: (state.height - baseHeight) / 2, baseWidth, baseHeight, scale: 100, opacity: 45, visible: true };
    references.push(reference);
    selectedReferenceId = reference.id;
} renderReferences(); setReferenceEditing(true); setStatus(`${files.length} reference image${files.length === 1 ? "" : "s"} added`); }
...
function bindExactRangeEditing() { const sizeInput = qs("#sizeInput"); sizeInput.addEventListener("input", event => { event.stopImmediatePropagation(); state.brush.size = sliderToBrushSize(Number(sizeInput.value)); syncBrushInputs(); }); const editor = document.createElement("div"), caption = document.createElement("span"), number = document.createElement("input"), apply = document.createElement("button"); editor.className = "range-editor"; editor.hidden = true; number.type = "number"; apply.type = "button"; apply.textContent = "Set"; editor.append(caption, number, apply); document.body.append(editor); let active = null; const close = () => { editor.hidden = true; active = null; }, commit = () => { if (!active)
    return; const min = Number(active.dataset.exactMin ?? active.min ?? "0"), max = Number(active.dataset.exactMax ?? active.max ?? "100"), step = Number((active.dataset.exactStep ?? active.step) || "1"), parsed = Number(number.value); if (!Number.isFinite(parsed))
    return; const value = clamp(Math.round((parsed - min) / step) * step + min, min, max); if (active.id === "layerOpacity")
    checkpoint("Layer opacity"); active.value = active.hasAttribute("data-log-size") ? String(brushSizeToSlider(value, max)) : String(value); active.dispatchEvent(new Event("input", { bubbles: true })); active.dispatchEvent(new Event("change", { bubbles: true })); setStatus(`${caption.textContent} set to ${value}`); close(); }; document.addEventListener("contextmenu", event => { const input = event.target.closest('input[type="range"]'); if (!input)
    return; event.preventDefault(); active = input; const min = Number(input.dataset.exactMin ?? input.min ?? "0"), max = Number(input.dataset.exactMax ?? input.max ?? "100"), label = input.closest("label")?.childNodes[0]?.textContent?.trim() || input.dataset.brushSetting || input.id || "Value", current = input.hasAttribute("data-log-size") ? state.brush.size : Number(input.value), rect = input.getBoundingClientRect(); caption.textContent = `${label} (${min}–${max})`; number.min = String(min); number.max = String(max); number.step = (input.dataset.exactStep ?? input.step) || "1"; number.value = String(current); editor.hidden = false; editor.style.left = `${clamp(rect.left, 6, innerWidth - editor.offsetWidth - 6)}px`; editor.style.top = `${Math.min(rect.bottom + 7, innerHeight - editor.offsetHeight - 6)}px`; number.focus(); number.select(); }); apply.addEventListener("click", commit); number.addEventListener("keydown", event => { if (event.key === "Enter") {
    ...
    event.preventDefault();
    close();
} }); document.addEventListener("pointerdown", event => { if (!editor.hidden && !editor.contains(event.target) && event.target !== active)
    close(); }, { capture: true }); window.addEventListener("keydown", event => { const target = event.target, mod = event.ctrlKey || event.metaKey, key = event.key.toLowerCase(); if (target.type !== "range" || !mod || (key !== "z" && key !== "y"))
...
async function openProject(file) { const project = JSON.parse(await file.text()); if (project.format !== "sogsketch")
    throw new Error("Not a SogSketch project"); await restoreSnapshot({ ...project.state, label: "Open project" }); state.brush = { ...state.brush, ...project.brush }; state.guides = project.guides; references = (project.references ?? []).map(item => ({ ...item })); nextReferenceId = Math.max(1, ...references.map(item => item.id + 1)); selectedReferenceId = references.at(-1)?.id ?? null; setReferenceEditing(false); renderReferences(); history.reset(); syncBrushInputs(); fitCanvas(); setStatus("Project opened"); }
function saveProject() { const project = { format: "sogsketch", version: 1, savedAt: new Date().toISOString(), state: (() => { const { label, ...rest } = snapshot("Save project"); return rest; })(), brush: { ...state.brush }, guides: [...state.guides], references: references.map(item => ({ ...item })) }; downloadBlob(new Blob([JSON.stringify(project)], { type: "application/json" }), `${safeName()}.sogsketch`); }
...
function scheduleAutosave() { if (!state.autosave || historyBusy)
    ...
    localStorage.setItem("sogsketch-recovery", JSON.stringify({ format: "sogsketch", version: 1, savedAt: new Date().toISOString(), state: saved, brush: state.brush, guides: state.guides, references }));
}
catch {
...
function createDocument() { const width = Number(qs("#newWidth").value), height = Number(qs("#newHeight").value); document.body.classList.remove("needs-document"); state.name = qs("#newName").value.trim() || "Untitled"; state.resolution = Number(qs("#newResolution").value) || 72; const background = qs("#newBackground").value; state.background = background === "custom" ? qs("#newBackgroundColor").value : background; state.layers = []; state.nextLayerId = 1; state.selection = null; references = []; selectedReferenceId = null; nextReferenceId = 1; setReferenceEditing(false); setCanvasSize(width, height); const layer = makeLayer("Paint Layer 1"); state.layers = [layer]; state.selectedLayerId = layer.id; history.reset(); updateTitle(); renderReferences(); renderLayers(); composite(); fitCanvas(); setStatus("New document"); }
...
Object.assign(commands, { addReference: () => referenceFileInput.click(), addGuideX: () => { const position = Number(prompt("Horizontal guide position", String(Math.round(state.height / 2)))); if (Number.isFinite(position)) {
...
function bindReferenceUi() { qs("#addReferenceImage").addEventListener("click", () => referenceFileInput.click()); referenceFileInput.addEventListener("change", async () => { await addReferenceFiles([...(referenceFileInput.files ?? [])]); referenceFileInput.value = ""; }); qs("#editReferences").addEventListener("click", () => setReferenceEditing(!referenceEditing)); referenceOpacity.addEventListener("input", () => { const reference = selectedReference(); if (!reference)
    return; reference.opacity = Number(referenceOpacity.value); renderReferences(); }); referenceScale.addEventListener("input", () => { const reference = selectedReference(); if (!reference)
    return; reference.scale = Number(referenceScale.value); renderReferences(); }); qs("#traceReference").addEventListener("click", () => { const reference = selectedReference(); if (!reference)
    return; reference.opacity = 35; renderReferences(); setReferenceEditing(false); }); qs("#toggleReferenceVisibility").addEventListener("click", () => { const reference = selectedReference(); if (!reference)
    return; reference.visible = !reference.visible; renderReferences(); }); qs("#deleteReference").addEventListener("click", deleteSelectedReference); document.addEventListener("keydown", event => { if (!referenceEditing)
    return; if (event.key === "Delete") {
    event.preventDefault();
...
async function init() { document.body.classList.add("needs-document"); setCanvasSize(Number(qs("#newWidth").value), Number(qs("#newHeight").value)); state.layers = []; state.selectedLayerId = null; bindUi(); bindReferenceUi(); bindBrushEngineUi(); document.addEventListener("keydown", keyHandler); syncView(); syncColors(); syncBrushInputs(); renderBrushPresets(); renderLayers(); updateTitle(); composite(); setTimeout(fitCanvas, 50); const workspace = JSON.parse(localStorage.getItem("sogsketch-workspace") ?? "null"); if (workspace?.dock === false)
