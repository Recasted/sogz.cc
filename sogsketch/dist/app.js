...
function keyHandler(event) { const target = event.target; if (["INPUT", "TEXTAREA", "SELECT"].includes(target.tagName))
    ...
    fitCanvas(); }
async function init() { setCanvasSize(state.width, state.height); const base = makeLayer("Paint Layer 1"); state.layers = [base]; state.selectedLayerId = base.id; bindUi(); document.addEventListener("keydown", keyHandler); syncView(); syncColors(); syncBrushInputs(); renderBrushPresets(); renderLayers(); updateTitle(); composite(); setTimeout(fitCanvas, 50); const workspace = JSON.parse(localStorage.getItem("sogsketch-workspace") ?? "null"); if (workspace?.dock === false)
    qs("#dock").classList.add("mobile-hidden"); if (workspace?.tools === false)
    qs("#toolbar").classList.add("mobile-hidden"); await recover(); }
