...
function bindUi() { document.querySelectorAll("[data-tool]").forEach(button => button.addEventListener("click", () => setTool(button.dataset.tool))); document.querySelectorAll("[data-tool-command]").forEach(button => button.addEventListener("click", () => setTool(button.dataset.toolCommand))); document.querySelectorAll("[data-command]").forEach(button => button.addEventListener("click", () => { const command = button.dataset.command; if (command)
    ...
    input.addEventListener("change", () => setForeground(rgbToHex(...hslToRgb(Number(hInput.value), Number(sInput.value), Number(lInput.value))))); qs("#swapColors").addEventListener("click", () => { [state.foreground, state.backgroundColor] = [state.backgroundColor, state.foreground]; syncColors(); }); qs("#resetColors").addEventListener("click", () => { state.foreground = "#000000"; state.backgroundColor = "#FFFFFF"; syncColors(); }); qs("#saveBrushPreset").addEventListener("click", saveBrushPreset); qs("#toolsToggle").addEventListener("click", () => qs("#toolbar").classList.toggle("mobile-hidden")); const dock = qs("#dock"); qs("#dockToggle").addEventListener("click", () => dock.classList.remove("mobile-hidden")); qs("#dockClose").addEventListener("click", () => dock.classList.add("mobile-hidden")); qs("#autosaveToggle").addEventListener("change", event => { state.autosave = event.target.checked; }); const newDialog = qs("#newDialog"); newDialog.addEventListener("cancel", event => { if (document.body.classList.contains("needs-document"))
