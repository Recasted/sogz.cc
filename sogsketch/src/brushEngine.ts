...
export class StrokeRenderer{
 ...
 private get usesStampPath():boolean{return effectTips.has(this.options.settings.tip)}
 begin(point:StrokePoint):void{this.previous={...point};this.filtered={...point};this.hasMoved=false;if(this.usesStampPath)this.paint(point,0,true)}
 add(point:StrokePoint):void{
  ...
  if(this.usesStampPath)this.emitSegment(this.filtered,target);else this.drawContinuous(this.filtered,target);this.hasMoved=true;
  this.previous={...point};this.filtered=target;
 }
 private drawStart(point:StrokePoint):void{
  ...
  this.ctx.save();this.options.clip?.(this.ctx);this.ctx.globalCompositeOperation=this.options.erase?"destination-out":"source-over";this.ctx.fillStyle=this.options.color;
  if(softness>.08&&!this.options.erase&&s.tip!=="flat"&&s.tip!=="chisel"){this.ctx.globalAlpha=alpha*(.42+.4*(1-softness));this.ctx.shadowColor=this.options.color;this.ctx.shadowBlur=radius*softness*.9}
  else this.ctx.globalAlpha=alpha;
  this.ctx.beginPath();if(s.tip==="flat"||s.tip==="chisel"){const angle=(s.angle+s.rotation)*Math.PI/180;this.ctx.ellipse(point.x,point.y,radius,Math.max(.5,radius*(s.tip==="chisel"?.24:.32)),angle,0,Math.PI*2)}else this.ctx.arc(point.x,point.y,radius*Math.max(.45,1-softness*.32),0,Math.PI*2);this.ctx.fill();this.ctx.restore()
 }
 private drawContinuous(a:StrokePoint,b:StrokePoint):void{
  ...
  this.ctx.save();this.options.clip?.(this.ctx);this.ctx.globalCompositeOperation=this.options.erase?"destination-out":"source-over";this.ctx.fillStyle=this.options.color;
  const fillBand=(scale:number):void=>{this.ctx.beginPath();this.ctx.moveTo(a.x+nx*wa*scale,a.y+ny*wa*scale);this.ctx.lineTo(b.x+nx*wb*scale,b.y+ny*wb*scale);this.ctx.lineTo(b.x-nx*wb*scale,b.y-ny*wb*scale);this.ctx.lineTo(a.x-nx*wa*scale,a.y-ny*wa*scale);this.ctx.closePath();if(s.tip==="flat"||s.tip==="chisel")this.ctx.ellipse(b.x,b.y,rb*scale,Math.max(.4,rb*(s.tip==="chisel"?.24:.32)*scale),angle,0,Math.PI*2,true);else this.ctx.arc(b.x,b.y,rb*scale,0,Math.PI*2,true);this.ctx.fill()};
  const softness=1-s.hardness/100;
  if(softness>.08&&!this.options.erase&&s.tip!=="flat"&&s.tip!=="chisel"){this.ctx.globalAlpha=alpha*(.42+.4*(1-softness));this.ctx.shadowColor=this.options.color;this.ctx.shadowBlur=Math.max(ra,rb)*softness*.9;fillBand(Math.max(.45,1-softness*.32));this.ctx.shadowBlur=0}
  else{this.ctx.globalAlpha=alpha;fillBand(1)}
  if((s.tip==="bristle"||s.texture!=="none")&&!this.options.erase){const textureStrength:{[K in BrushTexture]:number}={none:.16,paper:.10,grain:.16,chalk:.24,charcoal:.3,canvas:.13,wet:.08},strength=s.tip==="bristle"?Math.max(.2,textureStrength[s.texture]):textureStrength[s.texture],strands=s.tip==="bristle"?7:s.texture==="charcoal"||s.texture==="chalk"?5:3;this.ctx.shadowBlur=0;this.ctx.strokeStyle=this.options.color;this.ctx.lineCap="round";this.ctx.globalAlpha=alpha*strength;for(let index=0;index<strands;index++){const offset=(index-(strands-1)/2)/Math.max(1,strands-1)*1.35,offsetA=ra*offset,offsetB=rb*offset;this.ctx.lineWidth=Math.max(.35,(ra+rb)/(strands*(s.tip==="bristle"?2.2:3.2)));this.ctx.beginPath();this.ctx.moveTo(a.x+nx*offsetA,a.y+ny*offsetA);this.ctx.lineTo(b.x+nx*offsetB,b.y+ny*offsetB);this.ctx.stroke()}}
  this.ctx.restore();
 }
 end(point?:StrokePoint):void{
  if(point)this.add(point);
  if(!this.usesStampPath&&!this.hasMoved&&this.filtered)this.drawStart(this.filtered);
